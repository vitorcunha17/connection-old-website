...
class Acesso extends CI_Model {
	...
	public function verificaLogar($email, $senha) {
		...
		$consultaADM = $this->db->query('SELECT * FROM administrador WHERE email_administrador=' . $this->db->escape($email) . ' AND senha_administrador='. $this->db->escape($senha));
        ...
        if($consultaADM->num_rows() > 0) {
        	...
        	//existe um admin com esses dados, então retornar uma sessão
        	$dados = ['conta_status' => 'administracao', 'dadosUsuario' => $consultaADM->result()];
        	return $dados;

...
class Localizacao extends Upload {
    ...
    public function getEnderecoByArea($id) {
        $this->load->database();
        $quantidade = $this->db->query("SELECT id_candidato, nome, video_nome, foto_candidato, cu.nome_curso, num, rua, bairro, cidade, estado, candidato.curso FROM candidato INNER JOIN curso cu ON candidato.curso = cu.id INNER JOIN area a ON cu.id_area = $id WHERE status = 2");

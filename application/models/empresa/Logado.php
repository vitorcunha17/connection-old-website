...
class Logado extends CI_Model {
    ...
    public function listaEstado() {
        $this->load->database();
        $sql = $this->db->query("SELECT * FROM estado ORDER BY estado ASC");
        return $sql->result();
    }

    public function listaUniversidades() {
        $this->load->database();
        $sql = $this->db->query("SELECT * from universidades  WHERE cidade = 'Curitiba' UNION SELECT * from universidades WHERE cidade != 'Curitiba' ");
        return $sql->result();
    }

    public function listaCidade() {
        $this->load->database();
        $sql = $this->db->query("SELECT * FROM Municipio WHERE Uf IN ('PR') ORDER BY Nome");
        return $sql->result();
    }

    public function listaCursosPorIdArea($id) {
        $this->load->database();
        //$sql = $this->db->query("SELECT DISTINCT id, nome_curso, id_area FROM curso ORDER BY nome_curso ASC");
        $sql = $this->db->query("SELECT DISTINCT curso.id as curso_id, curso.nome_curso, curso.id_area, area.area_nome FROM curso INNER JOIN area ON curso.id_area = area.id WHERE curso.id_area = ".$this->db->escape($id) . "ORDER BY nome_curso ASC");
    ...
    public function listaCandidato() {
        $this->load->database();
        $sql = $this->db->query("SELECT cu.nome_curso, cu.id, cu.id_area, a.area_nome, a.id, candidato.nome,  candidato.curso, candidato.recomendados, candidato.video_nome, candidato.id_candidato, candidato.pcd FROM candidato INNER JOIN curso cu ON candidato.curso = cu.id INNER JOIN area a ON cu.id_area = a.id WHERE candidato.status = 2 ORDER BY candidato.nome ASC");
        return $sql->result();
    }
  ...
  public function candidatosSelecionados($candidatos) {
        $this->load->database();
        $sql = $this->db->query("SELECT cu.nome_curso, cu.id, candidato.nome, candidato.email, candidato.quemsou, candidato.situacao_curso, candidato.id_candidato, candidato.curso, candidato.video_nome, candidato.recomendados, candidato.curso, candidato.curso, candidato.recomendados, candidato.video_nome, candidato.id_candidato, candidato.pcd 
        FROM candidato

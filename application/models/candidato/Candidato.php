...
class Candidato extends Upload {
    ...
    public function listaVagas($area, $cargo) {
        $this->load->database();
        $sql = $this->db->query("SELECT * FROM area INNER JOIN curso cu ON cu.id_area = area.id INNER JOIN vagas va ON va.curso_vaga = cu.id WHERE area.id = " . $this->db->escape($area) . " AND va.status_vaga = 2 AND va.pcd = 'nao' AND va.cargo = " . $this->db->escape($cargo) . " ORDER BY va.id_vaga ASC");
        return $sql->result();
    }

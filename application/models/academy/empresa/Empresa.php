...
class Empresa extends Upload2 {
    ...
    public function cadastroAcademy($dados){
        //verifica se usuario existe
        if (SELF::verificaUsuario($dados['email']) >= 1) {
    ...
    public function cadastroAcademyCoordenador($dados){
        //verifica se usuario existe
        if (SELF::verificaCoordenador($dados['email_coord']) >= 1) {
            return 'Coordenador já cadastrado.';
        } else {
    ...
    public function verificaCoordenador($email) {

        $this->load->database();
        //verifica se usuaio existe
        $quantidade = $this->db->get_where('departamentos', ['email_coord' => $email]);
        return $quantidade->num_rows();
    }

    public function buscaAcademyCoordenador($empresa_id) {

        $this->load->database();
        $sql = $this->db->query("SELECT * FROM departamentos WHERE  id_universidade = " . $this->db->escape($empresa_id));
        return $sql->result();
    }

    public function buscaCoordenador($email) {

        $this->load->database();
        $sql = $this->db->query("SELECT * FROM departamentos WHERE  email_coord = " . $this->db->escape($email));
        return $sql->result();
    }
    ...
    public function pegaEmpresa($id) {
        ...
        $this->load->database();
        //verifica se email existe
        $sql = $this->db->query('SELECT * FROM empresa WHERE id_empresa =' . $this->db->escape($id));
        return $sql->result();
    }
    ...
    public function validando_plano($id_plano, $id_empresa) {
        ...
        if (SELF::verifica_plano_empresa($id_empresa[0]->plano_empresa >= 1)) { //ou seja, se existir um plano
            //verifica se o plano solicitado é mais caro ou mias barato que o plano que a empresa já possui
            if (SELF::verifica_plano_empresa($id_empresa)[0]->valor_planos > $plano[0]->valor_planos) { //ou seja, se o plano da empresa for maior que o plano solicitado, então vai existir uma taxa para migrar de plano

...
<body>
	...
	<section class="tabs-pagamentos" style="margin-bottom:80px;">
							...
							<div role="tabpanel" class="tab-pane active" id="cartao">
								...
								<div class="row">
									<div class="col-xs-12 col-sm-11 col-md-11 col-lg-11">
										...
										<!-- verifica se a empresa já possui um plano -->
										<?php if($this->session->userdata('pagamento_plano')['plano_existe']) { /* se existe um plano ativo */ ?>


												<p>Plano atual: <?= strtolower($this->session->userdata('pagamento_plano')['plano_atual']); ?></p>
												<p>Taxa da migração: R$:  <?= number_format($this->session->userdata('pagamento_plano')['taxa'],2,",","."); ?></p>
												<p>Novo plano: <?= strtolower($this->session->userdata('pagamento_plano')['nome_plano']); ?></p>
												<p>Valor: R$: <?= number_format($this->session->userdata('pagamento_plano')['valor_plano'],2,",","."); ?></p>
												<h4 style="padding: 10px 0">Valor total, plano + taxa: R$: <?= number_format($this->session->userdata('pagamento_plano')['total'],2,",","."); ?></h4>

										<?php } /* final da listagem para quem já possui plano ativo */ else { /* se não existir plano ativo */ ?>


										 <h4>Plano: <?= strtolower($this->session->userdata('pagamento_plano')['nome_plano']); ?></h4>
										 <h4>Valor: R$: <?= number_format($this->session->userdata('pagamento_plano')['valor_plano'],2,",","."); ?></h4>

										<?php } /* fina da listagem para quem não possui plano ativo */ ?>
									...
									<form method="post" id="form_pagamento" name="form_pagamento">
										...
										<input type="hidden" id="total_compra" value="<?= $this->session->userdata('pagamento_plano')['total']; ?>">
									</form>
		...
		<div class="cartaoAjax" data-cartao-ajax="<?= base_url('empresa/login/pagamentoCartaoPlano') ?>"></div>
